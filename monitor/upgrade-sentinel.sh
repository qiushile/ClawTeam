...
# 用途：升级 OpenClaw 源码到指定版本（不影响 Docker 容器）
# 作者：哨兵 (Sentinel)
# 创建：2026-03-30
#
...
create_backup() {
    local version=$1
    local backup_path="$BACKUP_DIR/sentinel-backup-${version}-$(date +%Y%m%d-%H%M%S)"
    ...
    if [ -d "$OPENCLAW_DIR" ]; then
        # 只备份配置和关键文件，避免备份整个目录占用过大空间
        mkdir -p "$backup_path"
        cp -r "$OPENCLAW_DIR/openclaw.json" "$backup_path/" 2>/dev/null || true
        cp -r "$OPENCLAW_DIR/.openclaw" "$backup_path/" 2>/dev/null || true
        cp -r "$OPENCLAW_DIR/.git" "$backup_path/" 2>/dev/null || true
        
        # 记录备份信息
        echo "备份时间：$(date)" > "$backup_path/BACKUP_INFO.txt"
        echo "备份版本：$version" >> "$backup_path/BACKUP_INFO.txt"
        echo "源目录：$OPENCLAW_DIR" >> "$backup_path/BACKUP_INFO.txt"
    fi
...
verify_upgrade() {
    ...
    if [ -f "openclaw.json" ]; then
        echo -e "${GREEN}配置文件:${NC} 存在"
    else
        warn "openclaw.json 不存在"
    fi
...
${BLUE}注意:${NC}
  - 此脚本只升级 /opt/openclaw 源码（哨兵自己）
  - 不影响 /opt/openclaw-team 的 Docker 容器
  - 升级前会自动创建备份到 /opt/openclaw/backups/
  - 需要 root 权限运行
