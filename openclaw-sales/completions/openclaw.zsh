...
_openclaw_ltm() {
  ...
  _arguments -C \
     \
    "1: :_values 'command' 'list[List memories]' 'search[Search memories]' 'stats[Show memory statistics]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (list) _openclaw_ltm_list ;;
        (search) _openclaw_ltm_search ;;
        (stats) _openclaw_ltm_stats ;;
      esac
      ;;
  esac
}
